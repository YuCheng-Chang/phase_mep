function [optimal_parameters, ga_output] = phastimate_optimize(EEG,passband, truephase, filter_objects_by_order, bounds_filter_order, bounds_window, bounds_edge, bounds_ar_order, bound_hilbertwindow)
...
problem.options = optimoptions('ga', 'PlotFcn', @gaplotbestf, 'PopulationSize', 100,'UseParallel', true,'UseVectorized',false);
...
% the ratio exp(1i*x)./exp(1i*y is a new complex exponential whose phase is equal to x-y

% ang_var_of_diff = @(x, y) 1-abs(mean(exp(1i*x)./exp(1i*y)));
obj_fun=@avg_error;
% get_phase1(EEG,TMS_loc,passband,varargin)
fprintf('EEG.xmin=%f sec\n',EEG.xmin);
assert(isequal(size(truephase),size(get_phase1(EEG,-EEG.xmin,passband,'doPlot',false,'fvPlot',false))),'invalid fitnessfcn');
problem.fitnessfcn = @(x) obj_fun(truephase, ...
    get_phase1(EEG,-EEG.xmin,passband, 'window',x(1),'filterOrd',x(2),...
    'edge', x(3),'arOrd', x(4), 'hilbertwindow',x(5),'fvPlot',false) ...
    );
...
problem.nvars = 5;
problem.intcon = 1:5;
problem.lb = [bounds_window(1), bounds_filter_order(1), bounds_edge(1), bounds_ar_order(1), bound_hilbertwindow(1)];
problem.ub = [bounds_window(2), bounds_filter_order(2), bounds_edge(2), bounds_ar_order(2), bound_hilbertwindow(2)];

problem.x0 = ceil(mean([problem.lb; problem.ub]));

% A*x ≤ b
problem.Aineq = [-1 3 0 0 0;-1 0 2 0 0]; % -1*window_length+3*filter_order≤-1 %window_length x(1) > 3 * filter_order x(2)
problem.bineq = [-1 ;-1];%-win+2*edge<=-1
...
optimal_parameters.hilbertwindow=x(5);
