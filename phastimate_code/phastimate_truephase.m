...
% fprintf('original data size (%4d,%4d)\n ',size(data));

% filter the data through each of the filter objects and apply Hilbert transform
data = cellfun(@(x) hilbert(filtfilt(x, double(data))), filter_objects, 'UniformOutput', false);
